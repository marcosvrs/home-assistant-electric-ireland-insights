...
async def test_recorder_task_action_error_propagates(recorder_mock, hass, mock_config_entry):
    """Recorder task callback errors reach the awaiting coroutine."""
    mock_config_entry.add_to_hass(hass)
    with (
        ...
        def fail(_recorder) -> None:
            raise RuntimeError("recorder callback failed")

        with pytest.raises(RuntimeError, match="recorder callback failed"):
            await coordinator._async_run_recorder_task(get_instance(hass), fail)
        await coordinator.async_close()


async def test_recorder_task_ignores_duplicate_completion_callbacks(hass, mock_config_entry):
    """A recorder task completion callback is safe when delivered twice."""
    mock_config_entry.add_to_hass(hass)
    with (
        patch("custom_components.electric_ireland_insights.coordinator.ElectricIrelandAPI"),
        patch("custom_components.electric_ireland_insights.coordinator.async_create_clientsession"),
        ...
        def queue_task(task) -> None:
            task.on_done(None)
            task.on_done(None)

        recorder.queue_task.side_effect = queue_task
        await coordinator._async_run_recorder_task(recorder, lambda _recorder: None)
        await coordinator.async_close()


async def test_legacy_external_statistics_are_renamed(recorder_mock, hass, mock_config_entry):
    """All legacy external statistic IDs retain their recorded data after hashing."""
    mock_config_entry.add_to_hass(hass)
    legacy_suffixes = (
        "consumption",
        "cost",
        "cost_discounted",
        "consumption_flat_rate",
        "cost_flat_rate",
        "cost_flat_rate_discounted",
        "consumption_off_peak",
        "cost_off_peak",
        "cost_off_peak_discounted",
        "consumption_mid_peak",
        "cost_mid_peak",
        "cost_mid_peak_discounted",
        "consumption_on_peak",
        "cost_on_peak",
        "cost_on_peak_discounted",
    )
    start = datetime(2026, 3, 23, tzinfo=UTC)

    for suffix in legacy_suffixes:
        statistic_id = f"{DOMAIN}:{ACCOUNT}_{suffix}"
        is_consumption = suffix.startswith("consumption")
        metadata = StatisticMetaData(
            has_sum=True,
            mean_type=StatisticMeanType.NONE,
            name=f"Electric Ireland {suffix} ({ACCOUNT})",
            source=DOMAIN,
            statistic_id=statistic_id,
            unit_of_measurement="kWh" if is_consumption else "EUR",
            unit_class="energy" if is_consumption else None,
        )
        async_add_external_statistics(
            hass,
            metadata,
            [
                ...
                StatisticData(start=start + timedelta(hours=1), state=2.0, sum=3.0),
            ],
        )
    ...
    account_hash = mock_config_entry.unique_id
    hashed_ids = {f"{DOMAIN}:{account_hash}_{suffix}" for suffix in legacy_suffixes}
    metadata = await get_instance(hass).async_add_executor_job(partial(get_metadata, hass, statistic_source=DOMAIN))
    assert hashed_ids <= metadata.keys()
    assert {metadata[statistic_id][1]["name"] for statistic_id in hashed_ids} == {
        f"Electric Ireland {suffix} ({account_hash})" for suffix in legacy_suffixes
    }
    assert not ({f"{DOMAIN}:{ACCOUNT}_{suffix}" for suffix in legacy_suffixes} & metadata.keys())
...
async def test_legacy_external_statistic_collision_keeps_hashed_rows(
    ...
    for statistic_id, name, total in (
        (legacy_id, f"Electric Ireland Consumption ({ACCOUNT})", 3.0),
        (hashed_id, f"Electric Ireland Consumption ({mock_config_entry.unique_id})", 10.0),
    ):
        async_add_external_statistics(
            hass,
            StatisticMetaData(
            ...
            [
                StatisticData(start=start, state=1.0, sum=total - 2.0),
                StatisticData(start=start + timedelta(hours=1), state=2.0, sum=total),
            ],
        )
...
async def test_bill_period_cache_cleared_on_meter_id_rediscovery(recorder_mock, hass, mock_config_entry):
    ...
    with (
        ...
        mock_api_instance = AsyncMock()
        new_bill_periods = [{"startDate": "2026-01-01T00:00:00Z", "endDate": "2026-01-01T00:00:00Z"}]
        _setup_api_mock(
            mock_api_instance,
            authenticate_return=(discovered_ids, discovered_ids),
            bill_periods=new_bill_periods,
            hourly_return=[],
        )
        mock_api_class.return_value = mock_api_instance
        ...
        assert coordinator._bill_periods == new_bill_periods
        assert coordinator._bill_periods_fetched_at is not None
...
# ---------------------------------------------------------------------------
# RED tests for per-day transient failure tolerance (coordinator changes pending)
# ---------------------------------------------------------------------------


async def test_refresh_tolerates_per_day_cannot_connect(recorder_mock, hass, mock_config_entry):
    """A transient CannotConnect for one lookback day must not abort the whole refresh."""
    mock_config_entry.add_to_hass(hass)
    ...
    with (
        ...
        mock_api_instance = AsyncMock()
        day_data = make_datapoints(1)
        _setup_api_mock(
            mock_api_instance,
            hourly_side_effect=[day_data, CannotConnect("timeout"), day_data, day_data],
        )
        mock_api_class.return_value = mock_api_instance
        ...
        assert result["datapoint_count"] == len(day_data) * 3
        assert result["import_error"] is None
        assert mock_api_instance.get_hourly_usage.call_count == LOOKUP_DAYS


async def test_backfill_tolerates_per_day_cannot_connect(recorder_mock, hass):
    """A transient CannotConnect for one backfill day must not abort the entire backfill."""
    entry = MockConfigEntry(
        domain="electric_ireland_insights",
        data={"username": "t@t.com", "password": "p", "account_number": ACCOUNT},
        unique_id=ACCOUNT_HASH,
    ...
    with (
        ...
        mock_api_instance = AsyncMock()
        fail_index = INITIAL_LOOKBACK_DAYS // 2
        day_counter = 0

        def _hourly_side_effect(*args, **kwargs):
            nonlocal day_counter
            if day_counter == fail_index:
                day_counter += 1
                raise CannotConnect("timeout")
            data = make_datapoints(1, base_ts=1774224000 + day_counter * 86400)
            day_counter += 1
            return data

        _setup_api_mock(mock_api_instance, hourly_side_effect=_hourly_side_effect)
        mock_api_class.return_value = mock_api_instance
        ...
        assert day_counter == INITIAL_LOOKBACK_DAYS
        assert entry.data.get("tariff_stats_initialized") is True
...
async def test_full_history_backfill_tolerates_per_day_cannot_connect(recorder_mock, hass):
    ...
    fake_now = datetime(2026, 1, 6, 12, 0, tzinfo=UTC)
    with (
        patch("custom_components.electric_ireland_insights.coordinator.ElectricIrelandAPI") as mock_api_class,
        patch("custom_components.electric_ireland_insights.coordinator.async_create_clientsession"),
        patch("custom_components.electric_ireland_insights.coordinator.dt_now", return_value=fake_now),
        ...
        mock_api_instance = AsyncMock()
        bill_periods = [
            {"startDate": "2026-01-01T00:00:00Z", "endDate": "2026-01-05T23:59:59Z"},
        ]
        fail_index = 2
        day_counter = 0
...
# ---------------------------------------------------------------------------
# Coverage gap tests: coordinator edge branches
# ---------------------------------------------------------------------------


async def test_all_days_connection_failure_raises_update_failed(recorder_mock, hass, mock_config_entry):
    """When every lookback day fails, CannotConnect is raised; a repeat failure skips the lost-connection log."""
    mock_config_entry.add_to_hass(hass)
    ...
    with (
        ...
        _setup_api_mock(
            mock_api_instance,
            hourly_side_effect=CannotConnect("down"),
        )
        mock_api_class.return_value = mock_api_instance
        ...
        # First failure: was_successful is True, logs "Connection lost"
        with pytest.raises(UpdateFailed):
            await coordinator._async_update_data()
        # Second consecutive failure: was_successful is False, skips the log branch
        with pytest.raises(UpdateFailed):
            await coordinator._async_update_data()
...
async def test_update_data_cached_ids_invalid_without_discovery(recorder_mock, hass):
    """CachedIdsInvalid retry where re-auth discovers no new IDs leaves entry data untouched."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            "username": "t@t.com",
    ...
    with (
        ...
        mock_api_instance = AsyncMock()
        cached_ids = {"partner": "P0", "contract": "C0", "premise": "PR0"}
        new_ids = {"partner": "P9", "contract": "C9", "premise": "PR9"}
        # Second authenticate returns discovered_ids=None -> entry must not be updated
        mock_api_instance.authenticate = AsyncMock(side_effect=[(cached_ids, None), (new_ids, None)])
        mock_api_instance.get_bill_periods = AsyncMock(return_value=[])
        mock_api_instance.get_hourly_usage = AsyncMock(
            side_effect=[CachedIdsInvalid("stale")] + [make_datapoints(1)] + [[] for _ in range(LOOKUP_DAYS)]
        )
    ...
    updated = hass.config_entries.async_get_entry(entry.entry_id)
    assert updated is not None
    assert updated.data["partner_id"] == "P0"
    assert result["datapoint_count"] == 24


async def test_async_close_is_idempotent(recorder_mock, hass, mock_config_entry):
    """async_close returns early when the coordinator is already closed."""
    mock_config_entry.add_to_hass(hass)

    session_path = "custom_components.electric_ireland_insights.coordinator.async_create_clientsession"
    with (
        patch("custom_components.electric_ireland_insights.coordinator.ElectricIrelandAPI"),
        patch(session_path) as mock_session_factory,
    ):
        from custom_components.electric_ireland_insights.coordinator import ElectricIrelandCoordinator
...
async def test_tariff_backfill_bounds_lookback_by_bill_periods(recorder_mock, hass):
    """30-day backfill with bill periods fetches only dates inside the periods."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={"username": "t@t.com", "password": "p", "account_number": ACCOUNT},
        unique_id=ACCOUNT_HASH,
    ...
    fake_now = datetime(2026, 3, 15, 12, 0, tzinfo=UTC)
    bill_periods = [
        {"startDate": "2026-03-14T00:00:00Z", "endDate": "2026-03-14T23:59:59Z"},
    ]
    ...
    with (
        ...
        _setup_api_mock(
            mock_api_instance,
            bill_periods=bill_periods,
            hourly_return=make_datapoints(1),
        )
        mock_api_class.return_value = mock_api_instance
    ...
    # Intersection of the 30-day lookback with the single-day period is one day
    assert mock_api_instance.get_hourly_usage.call_count == 1
    assert entry.data.get("tariff_stats_initialized") is True


async def test_tariff_backfill_reauth_failure_creates_issue(recorder_mock, hass):
    """Backfill aborts with a repair issue when re-authentication after CachedIdsInvalid fails."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={"username": "t@t.com", "password": "p", "account_number": ACCOUNT},
        unique_id=ACCOUNT_HASH,
    ...
    with (
        ...
        mock_api_instance = AsyncMock()
        mock_api_instance.authenticate = AsyncMock(side_effect=[(TEST_METER_IDS, None), CannotConnect("down")])
        mock_api_instance.get_bill_periods = AsyncMock(return_value=[])
        mock_api_instance.get_hourly_usage = AsyncMock(side_effect=[CachedIdsInvalid("stale")])
        mock_api_class.return_value = mock_api_instance
    ...
    assert registry.async_get_issue(DOMAIN, f"backfill_auth_failed_{ACCOUNT_HASH}") is not None


async def test_none_value_datapoints_produce_no_statistics(recorder_mock, hass, mock_config_entry):
    """Datapoints with None consumption/cost are skipped; an all-None batch inserts nothing."""
    mock_config_entry.add_to_hass(hass)

    none_dps = [
        {"consumption": None, "cost": None, "start": 1774224000 + hour * 3600, "tariff_bucket": "off_peak"}
        for hour in range(24)
    ]
    ...
    with (
        ...
        _setup_api_mock(
            mock_api_instance,
            hourly_side_effect=[none_dps] + [[] for _ in range(LOOKUP_DAYS)],
        )
        mock_api_class.return_value = mock_api_instance
    ...
    stats = await get_instance(hass).async_add_executor_job(
        ...
        {STAT_ID_CONSUMPTION, STAT_ID_COST},
        "hour",
        None,
        {"sum", "state"},
    )
...
async def test_zero_discount_clears_existing_discounted_statistics(
    recorder_mock,
    hass,
    mock_config_entry,
):
    ...
    tariff_stat = f"{DOMAIN}:{ACCOUNT_HASH}_cost_off_peak_discounted"
    start = datetime(2026, 3, 23, tzinfo=UTC)
    statistics_to_seed = (
        (STAT_ID_COST_DISCOUNTED, "Electric Ireland Cost Discounted"),
        (legacy_account_stat, f"Electric Ireland Cost Discounted ({ACCOUNT})"),
        (tariff_stat, "Electric Ireland Cost Off-Peak Discounted"),
        (STAT_ID_COST, "Electric Ireland Cost"),
        (other_account_stat, "Other Cost Discounted"),
    )

    for statistic_id, name in statistics_to_seed:
        async_add_external_statistics(
            hass,
            StatisticMetaData(
                has_sum=True,
    ...
    metadata = await get_instance(hass).async_add_executor_job(partial(get_metadata, hass, statistic_source=DOMAIN))
    removed_ids = {STAT_ID_COST_DISCOUNTED, legacy_account_stat, tariff_stat}
    assert not removed_ids & metadata.keys()
    assert {STAT_ID_COST, other_account_stat} <= metadata.keys()
